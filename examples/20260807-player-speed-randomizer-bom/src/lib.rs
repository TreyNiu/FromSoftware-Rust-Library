...
use crate::{
    config::{
        PlayerSpeedRandomizerBomConfig, config_modified_time, load_config, load_or_create_config,
        resolve_paths,
    },
    html_status::HtmlStatusWriter,
    speed_randomizer::PlayerSpeedRandomizer,
};
...
struct State {
    ...
    randomizer: PlayerSpeedRandomizer,
    html: HtmlStatusWriter,
}

impl State {
    fn new(
        config: PlayerSpeedRandomizerBomConfig,
        config_path: PathBuf,
        html_path: PathBuf,
    ) -> Self {
        Self {
            ...
            randomizer: PlayerSpeedRandomizer::new(&config.speed, INPUT_CHECK_INTERVAL),
