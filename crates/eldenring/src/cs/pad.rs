...
///    .ok()
///    .and_then(|pad_man: &FD4PadManager| pad_man.get_in_game_pad())
/// {
...
impl CSPad {
    pub fn poll_digital_input(&self, input_code: UserInputKey) -> bool {
        ...
        if let Some(state) = self.unused_input_map.find(&input_code)
            && *state
        {
            return true;
        }
    ...
    fn index_digital_input(&self, virtual_input_index: i32) -> bool {
        ...
        if let Some(mut mouse_button_states_index) = key_assign
            ...
            .find(&virtual_input_index)
            .filter(|state_index| **state_index != -1)
            .cloned()
        {
            mouse_button_states_index -= 1000;
            if mouse_button_states_index > 0 && mouse_button_states_index <= 80 {
