...
const DATA_BLOCK_START: &str =
    r#"<script id="enemy-speed-randomizer-data" type="application/json">"#;
...
impl HtmlStatusWriter {
    ...
    pub fn write_if_due(&mut self, status: SpeedStatus) {
        ...
        let updated = if status.individual_enemy_speed {
            replace_countdown(&template, status)
        } else {
            replace_data_block(&template, status)
        };
...
fn replace_countdown(template: &str, status: SpeedStatus) -> Option<String> {
    let start = template.find(DATA_BLOCK_START)?;
    let content_start = start + DATA_BLOCK_START.len();
    let content_end = content_start + template[content_start..].find(DATA_BLOCK_END)?;
    let field_name = "\"countdown\"";
    let field_start = content_start + template[content_start..content_end].find(field_name)?;
    let value_search_start = field_start + field_name.len();
    let colon_start = value_search_start + template[value_search_start..content_end].find(':')?;
    let quote_search_start = colon_start + 1;
    let value_start = quote_search_start + template[quote_search_start..content_end].find('"')? + 1;
    let value_end = value_start + template[value_start..content_end].find('"')?;
...
<title>Enemy Speed Randomizer</title>
<style>
...
<script id="enemy-speed-randomizer-data" type="application/json">
{"enabled":false,"speedEnabled":false,"multiplier":1.0,"countdown":"--:--"}
</script>
<script>
(() => {
 const data = JSON.parse(document.getElementById("enemy-speed-randomizer-data").textContent);
...
mod tests {
    ...
    #[test]
    fn individual_mode_only_replaces_countdown() {
        let template = format!(
            "<style>custom</style>{DATA_BLOCK_START}\n{{\"enabled\":true,\"speedEnabled\":true,\"multiplier\":1.500,\"countdown\":\"old\"}}\n{DATA_BLOCK_END}"
        );
        let updated = replace_countdown(
            &template,
            SpeedStatus {
                enabled: false,
                speed_enabled: false,
                multiplier: 1.0,
                countdown_ms: Some(3_890),
                individual_enemy_speed: true,
            },
        )
        .unwrap();

        assert!(updated.contains("\"enabled\":true"));
        assert!(updated.contains("\"speedEnabled\":true"));
        assert!(updated.contains("\"multiplier\":1.500"));
        assert!(updated.contains("\"countdown\":\"00:03\""));
        assert!(updated.contains("<style>custom</style>"));
    }
